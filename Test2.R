...
## attempting to regularise input stream
...
sp1 <-
  siteParams %>%
  transmute(width = AvgWettedWidth_m,
            depth = AvgWettedDepth_cm / 100,
            InjectionTime = as_hms(InjectionTime), # assumes all sampling within one 24h period
            distance = Reach_Length_meters,
            discharge = Discharge_LitersPerSec,
            # Calculate injected Cl in g and N in mg
            inject_Cl = Injected_NaCl_g * 35.5 / (35.5 + 23) +
              Injected_NH4Cl_g * 35.5 / (14 + 4 + 35.5),
            inject_N = Injected_NH4Cl_g * 14 / (14 + 4 + 35.5) * 1000
            )
...
# Assuming stock solution is released over one segment 0.5m
initial.length <- 1

# Injectate concentration immediately after releasing. Cl in g/m3 and N in mg/m3#
Cl.ini <- inject_Cl / (width * depth * initial.length * 0.5)
N.ini <- inject_N / (width * depth * initial.length * 0.5)
...
data <- relDat %>%
  transmute(collect_time = as_hms(CollectionTime),
            time = collect_time - sp1$InjectionTime,
            Cl = ObservedCl_mgL,
            N = ObservedNH4N_ugL)
...
dataL <-
  data %>%
  pivot_longer(cols = c("Cl", "N"),
               names_to = "chem",
               values_to = "conc")

ggplot(data=dataL, aes(time, conc, colour = chem)) + geom_point()
